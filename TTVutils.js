...
export async function use_code(code) {
  const body = `client_id=${process.env.TTVclientID}&` +
...
export async function TTVinfo() {
  ...
  return { title: title, game: game, art: `${art.split("-{width}x{height}")[0]}.jpg`}
}
...
function saveToken(data) {
  ...
  db.set("token", `Bearer ${access}`)
  db.set("refresh", refresh)
}

async function refreshToken() {
  const refresh = await db.get("refresh")  
